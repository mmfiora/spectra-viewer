...
class SpectraViewerFluo(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Spectra Viewer (Fluorescence)")
        ...
        # Load files
        self.load_btn = QPushButton("Load .txt Files")
        ...
        # Plot selected columns button (below delete)
    ...
    def load_file(self):
        path, _ = QFileDialog.getOpenFileName(self,"Open File","","Data Files (*.txt *.csv)")        
        if not path:
            return
        try:
            spectra = load_fluo_file(path)
    ...
    def update_plot(self):
        if not self.plotted_curves:
            ...
            return
        fig = plot_spectra(self.plotted_curves, y_label="Fluorescence (a.u.)")
    ...
    def save_plot_as_jpg(self):
        ...
        fig = plot_spectra(self.plotted_curves, y_label="Fluorescence (a.u.)")
        export_plot_as_jpg(self.plotted_curves, path, y_label="Fluorescence (a.u.)")
